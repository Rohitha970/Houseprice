...
/* ━━━ ALL TEXT DARK BY DEFAULT (dashboard) ━━━ */
...
.sec-head {
    ...
    background: linear-gradient(120deg,#e8efff,#d4e3ff) !important;
    border-left: 4px solid #1648ff !important;
    padding: 10px 18px !important;
    border-radius: 0 12px 12px 0 !important;
    margin: 20px 0 12px !important;
}
.sec-head * { color: #1648ff !important; }
...
.stTabs [data-baseweb="tab"] * { color: #5a6a8a !important; }
...
.stSelectbox label *,
.stNumberInput label *,
[data-testid="stToggleLabel"] * { color: #0d1f3c !important; }

/* ━━━ TEXT INPUTS — ALWAYS DARK TEXT ON WHITE ━━━ */
...
/* ━━━ SELECTBOX — DARK TEXT ON WHITE, NO EXTRA BORDERS ━━━ */
.stSelectbox > div > div {
    border-radius: 12px !important;
    border: 2px solid #dde3f5 !important;
    background: white !important;
...
.stSelectbox > div > div * { color: #0d1f3c !important; }
[data-baseweb="select"] { border: none !important; }
[data-baseweb="select"] * { color: #0d1f3c !important; }
...
[data-testid="stToggleLabel"] span { color: #0d1f3c !important; font-weight: 600 !important; }
...
[data-testid="metric-container"] label { color: #5a6a8a !important; font-size: 11px !important; font-weight: 700 !important; text-transform: uppercase !important; }
[data-testid="stMetricValue"] { color: #1648ff !important; font-size: 22px !important; font-weight: 900 !important; }
[data-testid="stMetricDelta"] { color: #00c896 !important; }

/* ━━━ CAPTIONS ━━━ */
.stCaption, [data-testid="caption"] { color: #5a6a8a !important; font-size: 13px !important; }
...
/* ━━━ EXPANDER ━━━ */
.streamlit-expanderHeader { color: #1648ff !important; background: #e8efff !important; border-radius: 12px !important; font-weight: 700 !important; }
...
[data-testid="column"] { min-width: 0 !important; width: auto !important; flex: 1 1 auto !important; }

/* ━━━ MOBILE ≤768px ━━━ */
@media (max-width: 768px) {
    [data-testid="column"] { width: 100% !important; flex: 1 1 100% !important; min-width: 100% !important; }
    .main .block-container { padding: 0.4rem 0.6rem !important; }
    .stTextInput input, .stNumberInput input, .stSelectbox > div > div { font-size: 16px !important; min-height: 48px !important; }
...
if not st.session_state.logged_in:
    ...
    /* Every element white on dark login bg */
    p, span, div, label, h1, h2, h3, h4, h5, h6, li,
    [data-testid], [class*="st-"] { color: white !important; }
    /* Input labels */
    .stTextInput label, .stTextInput label * { color: white !important; font-weight: 700 !important; }
    /* Input boxes — semi-transparent with white text */
    .stTextInput input {
    ...
    .stTextInput input::placeholder { color: rgba(255,255,255,0.55) !important; }
    .stTextInput input:focus { border-color: #00d4ff !important; }
    /* Buttons on login */
    .stButton > button {
...
def get_history():
    try:
        with engine.connect() as con:
            df = pd.read_sql(
                text("SELECT * FROM predictions ORDER BY timestamp DESC"), con)
        return df
    except Exception:
        return pd.DataFrame()
...
with tab_val:
    ...
    # Logout placed right above valuation button
...
with tab_analytics:
    ...
    if df_hist.empty:
        ...
        # Axis style — explicit tickfont so labels are always visible
        AXIS = dict(
            color="#0d1f3c",
            tickfont=dict(color="#0d1f3c", size=11),
            title_font=dict(color="#0d1f3c", size=12),
            linecolor="#0d1f3c",
            automargin=True,
        )
        ...
        LAYOUT = dict(
            paper_bgcolor="white",
            plot_bgcolor="white",
            font=dict(family="Plus Jakarta Sans", color="#0d1f3c", size=12),
            margin=dict(l=40, r=16, t=52, b=60),
            height=360,
        ...
        fig1.update_layout(**LAYOUT,
                           xaxis={**AXIS, "tickangle":-40, "gridcolor":"#e8efff"},
                           yaxis={**AXIS, "gridcolor":"#e8efff"})
        ...
        fig2.update_layout(**LAYOUT,
                           xaxis={**AXIS, "gridcolor":"#f0e8ff"},
                           yaxis={**AXIS, "gridcolor":"#f0e8ff"})
        ...
        fig3.update_layout(**LAYOUT, showlegend=False,
                           xaxis={**AXIS, "gridcolor":"#fff3b0"},
                           yaxis={**AXIS, "gridcolor":"#fff3b0"})
        ...
        fig4.update_layout(**LAYOUT, showlegend=False,
                           xaxis={**AXIS, "gridcolor":"#c8f5e8"},
                           yaxis={**AXIS, "gridcolor":"#c8f5e8"})
